...
function getScreenStreamConstraints(
  ...
  if (sourceId === "browser-display-media") {
    ...
    if (profile.height > 0) {
      ...
      video.width = { ideal: Math.round((profile.height * 16) / 9) };
    }
...
async function openStreamSetup({ reuseCurrent = false } = {}) {
  ...
  try {
    let sources = (await window.aeroChat?.getScreenSources?.()) || [];
    if (!sources.length && navigator.mediaDevices?.getDisplayMedia) {
      sources = [
        {
          id: "browser-display-media",
          name: "System screen picker",
          displayId: "",
          thumbnail: "",
          appIcon: "",
          type: "screen",
