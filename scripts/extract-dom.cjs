...
const indexJsPath = path.join('c:\\Dev\\nodejs\\AeroP2Pchat', 'src', 'renderer', 'index.js');
const domJsPath = path.join('c:\\Dev\\nodejs\\AeroP2Pchat', 'src', 'renderer', 'dom.js');
...
for (let i = 8; i < 250; i++) {
  const line = lines[i];
  if (line.startsWith('const ') && (line.includes('document.querySelector') || line.includes('document.getElementById') || line.includes('Array.from(document.querySelectorAll'))) {
    if (startIdx === -1) startIdx = i;
    endIdx = i;
  } else if (startIdx !== -1 && (line.trim() === '' || line.startsWith('  ') || line.startsWith(');'))) {
    // might be multi-line query selector
    endIdx = i;
  } else if (startIdx !== -1 && !line.startsWith('const ') && line.trim() !== '') {
    // end of DOM block
    break;
  }
}
